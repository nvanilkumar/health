...
class PageNate {
	...
	function getPaging() {
		...
		{		
			$_PARAMS[substr($value,0,strrpos($value,"="))]=substr($value,strrpos($value,"=")+1,strlen($value));
		}
		
        if($start_loop>1){
            $lastspan=($start_loop-$displayed_span>0)?$start_loop-$displayed_span:1;
			$onchange="changeTab(\"List\",\"".$_PARAMS['sort']."\",\"".$_PARAMS['order']."\",\"".$lastspan."\",\"".$_PARAMS['locality_name']."\",\"".$_PARAMS['village_name']."\",\"".$_PARAMS['hh_head_fname']."\");";
            $html .= "\n<a onclick=\"$onchange\" ><strong>&laquo;</strong></a>";
        }
		...
		for ($i=$start_loop; $i <= $stop_loop; $i++) {
			...
			if ($i == $this_page) {
				...
				$onchange="changeTab(\"List\",\"".$_PARAMS['sort']."\",\"".$_PARAMS['order']."\",\"".$i."\",\"".$_PARAMS['locality_name']."\",\"".$_PARAMS['village_name']."\",\"".$_PARAMS['hh_head_fname']."\");";				
				//echo $onchange;
				$html .= "\n<a onclick='".$onchange."' ><b>$i</b></a>";
			}
		}
        if($stop_loop<$total_pages){
            $nextspan=($stop_loop+$displayed_span>$total_pages)?$total_pages-$displayed_span:$stop_loop+$displayed_span;
			$onchange="changeTab(\"List\",\"".$_PARAMS['sort']."\",\"".$_PARAMS['order']."\",\"".$nextspan."\",\"".$_PARAMS['locality_name']."\",\"".$_PARAMS['village_name']."\",\"".$_PARAMS['hh_head_fname']."\");";
            $html .= "\n<a onclick=\"$onchange\" ><strong>&raquo;</strong></a>";
        }
